...
/**
 * Various methods to find process definitions by group and
 * {@link com.da.activiti.model.document.DocType1}. Be aware that all BPMN docs
 * must have a namepsace equal to {@code da.com} or whatever is set in the
...
public class WorkflowService {
	...
	@Autowired
	protected RepositoryService repoSrvc;
	@Autowired
	...
	/**
	 * @param processId
	 *            the process <strong>Definition</strong> Id - NOT the process
	 *            Instance Id.
	 * @return png image of diagram - nothing highlighted since this is the
	 *         process definition - not a specific instance.
	 */
	public byte[] getProcessDefinitionDiagram(String processId) throws IOException {
		ProcessDefinition pd = this.repoSrvc.createProcessDefinitionQuery().processDefinitionKey(processId)
	...
	public byte[] getActiveDocumentDiagram(String docId) throws IOException {
		...
		ProcessInstance pi = runtimeService.createProcessInstanceQuery().processInstanceBusinessKey(docId)
				.singleResult();
		...
		InputStream in = new DefaultProcessDiagramGenerator().generateDiagram(bpmnModel, "png",
				runtimeService.getActiveActivityIds(pi.getProcessInstanceId()));
		// InputStream in =
	 ...
	 * @param docType
	 * @param group
	 * @return true if the specific {@code DocType} and Group workflow exists,
	 *         false if not e.g. process Id of deployed definition would equal
	 *         {@code BOOK_REPORT_engineering}
	 */
	...
	/**
	 *
	 * @return the list (possibly empty) of DocTypes that have at least a base
	 *         type (i.e. group = NONE) defined.
	 */
	public List<String> findExistingBaseDocTypes() {
		...
		List<ProcessDefinition> processDefinitions = this.repoSrvc.createProcessDefinitionQuery()
				.processDefinitionCategory(WFConstants.NAMESPACE_CATEGORY).latestVersion()
		...
		for (ProcessDefinition processDefinition : processDefinitions) {
			...
			/*
			 * DocType docType = null; try { docType =
			 * DocType.valueOf(parsed[0]);
			 * 
			 * } catch(Exception e) { LOG.error(
			 * "Invalid docType detected in workflow (ignoring): {}",
			 * parsed[0]); } if (docType != null){ docTypes.add(docType);
			 * 
			 * }
			 */
		}
	...
	/**
	 * @param docType
	 * @return true if the base workflow for the {@code DocType} exists.
	 *         Essentially, this is a process definition with the
	 *         {@code processId=DOC_TYPE_NONE} and the namespace set to default
	 *         namespace
	...
	/**
	 * @param docType
	 * @return latest process definition for the given doc type and group or
	 *         null if none exits.
	 */
	public ProcessDefinition findProcDefByDocTypeAndGroup(String docType, String group) {
		...
		ProcessDefinition pd = this.repoSrvc.createProcessDefinitionQuery()
				.processDefinitionCategory(WFConstants.NAMESPACE_CATEGORY).processDefinitionKey(processIdStr)
				.latestVersion().singleResult();
		return pd;
	}
	...
	/**
	 * @param docType
	 * @return the base workflow for a given document type (i.e. the process id
	 *         of the process is {@code DocType_NONE}) or null if no base
	 *         document exists.
	 */
	 ...
	 *            retrieve only the latest version of each Process Definition
	...
	public void updateWorkflow(BpmnModel model, String group) {
		...
		Deployment deployment = this.repoSrvc.createDeployment().addBpmnModel(modelName, model).name(deployName)
				.deploy();
	}

	/**
	 * @param businessKey
	 *            the document Id as returned from DAO classes
	...
	/**
	 * @param docType
	 * @return returns the matching {@code ProcessDefinition} based on the
	 *         namespace (i.e. category) which must be a valid
	 *         {@link com.da.activiti.model.document.DocType} or null if no
	 *         definition exists.
	 */
	/*
	 ...
	 * repoSrvc.createProcessDefinitionQuery().processDefinitionCategory(docType
	 * ).singleResult(); return pd;
	 * 
	 * }
	 */
	/**
	 * @param group
	 *            - If no {@code Group} is passed, the default document approval
	 *            workflow will be used.
	 * @return a sorted list of approvals contained in the workflow associated
	 *         with the given group
	 */
	/*
	 * public List<UserTask> getDocApprovalsByGroup(String group) { String base
	 * = "FOO";//TODOWorkflow.PROCESS_ID_DOC_APPROVAL; String procId =
	 * StringUtils.isBlank(group) ? base : base + "-" + group; LOG.debug(
	 * "building approval list for procDef: " + procId); ProcessDefinition pd =
	 * this.repoSrvc.createProcessDefinitionQuery().processDefinitionKey(procId)
	 * .latestVersion().singleResult(); BpmnModel model =
	 * this.repoSrvc.getBpmnModel(pd.getId()); org.activiti.bpmn.model.Process
	 * process = model.getProcesses().get(0);
	 * 
	 * SubProcess sub = (SubProcess)
	 * process.getFlowElement(Workflow.SUBPROCESS_ID_DYNAMIC);
	 * LOG.debug(sub.getName()); Collection<FlowElement> flowElements =
	 * sub.getFlowElements(); List<org.activiti.bpmn.model.UserTask> userTasks =
	 * Lists.newArrayList(); for (FlowElement el : flowElements) { if
	 * (el.getClass().equals(org.activiti.bpmn.model.UserTask.class)) {
	 * userTasks.add((org.activiti.bpmn.model.UserTask) (el)); } }
	 * 
	 * List<UserTask> approvals = Lists.newArrayList(); int i = 1; for
	 * (org.activiti.bpmn.model.UserTask userTask : userTasks) {
	 * approvals.add(fromUserTask(userTask, i)); i++; } return approvals; }
	 */
	...
	public Process createProcess(String processName, List<ProcessInfo> subprocess) throws IOException {
		...
		ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(deploymentId);
	...
	/*
	 * public Process createProcess(String processName,List <ProcessInfo>
	 * subprocess) throws IOException { // 1. Build up the model from scratch
	 * BpmnModel model = new BpmnModel(); Process process = new Process();
	 * model.addProcess(process); model.setTargetNamespace("da.com"); // String
	 * deploymentId = processName+"___NONE"; String deploymentId =
	 * processName+"___NONE"; process.setId(deploymentId);
	 * 
	 * process.addFlowElement(createStartEvent());
	 * process.addFlowElement(createUserTask("task1", "First task", "fred"));
	 * process.addFlowElement(createUserTask("task2", "Second task", "john"));
	 * process.addFlowElement(createEndEvent());
	 * 
	 * process.addFlowElement(WorkflowBuilder.createSequenceFlow("start",
	 * "task1"));
	 * process.addFlowElement(WorkflowBuilder.createSequenceFlow("task1",
	 * "task2"));
	 * process.addFlowElement(WorkflowBuilder.createSequenceFlow("task2",
	 * "end"));
	 * 
	 * // 2. Generate graphical information new BpmnAutoLayout(model).execute();
	 * 
	 * // 3. Deploy the process to the engine Deployment deployment =
	 * this.repoSrvc.createDeployment()
	 * .addBpmnModel(deploymentId+"dynamic-model.bpmn", model).name(
	 * "Dynamic process deployment") .deploy();
	 * 
	 * 
	 * ProcessDefinition processDefinition =
	 * this.repoSrvc.createProcessDefinitionQuery().
	 * processDefinitionKey(deploymentId).latestVersion().singleResult();
	 * 
	 ...
	 * 
	 * 
	 * // 6. Save process diagram to a file InputStream processDiagram =
	 ...
	 * File("target/"+deploymentId+"process.bpmn20.xml"));
	 * 
	 * return process; }
	 */
	...
	public String getProcesDefinationByProcessName(String processName) {
		...
		ProcessDefinition pd = this.repoSrvc.createProcessDefinitionQuery().processDefinitionKey(processName)
				.latestVersion().singleResult();
